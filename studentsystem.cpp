...
public:
    ...
    {
        stringstream ss;
        ss << rollNo << "," << name << "," << marks[0] << "," << marks[1] << "," << marks[2];
        return ss.str();
    }
...
int main()
{
    ...
    try
    {
        ...
        if (s1 && s2)
        {
            cout << "\nComparison:" << endl;
            if (*s1 > *s2)
                cout << s1->name << " has higher average than " << s2->name << endl;
            else
                cout << s2->name << " has higher average than " << s1->name << endl;
        }
        ...
        // Save happens automatically on destruction
    }
